...
> 이 파일과 `CLAUDE.md`, 그리고 `README.md`(루트·양쪽 앱) 만 한국어다. **커밋되어 GitHub에
> 올라가는 나머지 전부 — 코드, 주석, 워크플로, 커밋 메시지 — 는 영어로만 쓴다.** 사용자와의
> 대화는 한국어.
>
> README 는 한국어 `README.md` 가 기본이고 영어판은 옆의 `README.en.md` 다. 둘은 맨 위의
> 언어 전환 링크로 서로 가리킨다. **README 를 고치면 두 언어판을 함께 고친다.**
