 ...
 * through `local.properties`/CI rather than hardcoded, so it never lands in git.
 *
 ...
 * the real partition could never be undone.
 */
object DropboxConfig {
     ...
     * Dropbox app console. The claim in an earlier task plan — that the scheme "splits
     * dev and release automatically" — is wrong, since the key is shared on purpose.
     */
    val redirectUri: String get() = "db-$appKey://1/connect"

    /**
     * Scopes: files.content.write is for two-way sync (AGENTS.md §1). A link made before it was
     * requested keeps working read-only until the user reconnects; see [canWrite].
     */
    ...
    /** Shared secret written by Desktop and read by [SecretStore]; the path is defined here so both agree. */
