...
- **(양방향 목표)** 업로드까지 포함한 순서는 **전처리 → 등록 → 업로드**다. Dropbox 에는
  ...
  - SAF 는 원자적 교체가 없어서, Android 는 원본을 `.flonovel/original/` 에 백업한 뒤 숨김 임시
    ...
    파일 URI 가 바뀌므로 책을 **처음 열기 전**(또는 처음 올리기 전)에만 한다.
...
- `feature/two-way-sync` 브랜치에 양방향 동기화 전체(양쪽 엔진, Android 전처리, 동기화 시점과
  UI, 이동 감지)가 들어가 있다. 실기기 확인 뒤 한 번에 머지한다. 머지하면 이 "현재" 항목을 지운다.
