 ...
 * Dropbox's published algorithm). The Android test suite pins the same constants, so the two
...
class ContentHashTest {
    ...
    @Test
    fun fileAndBytes_agree() {
        val data = pattern(block * 2 + 3)
        val file = Files.createTempFile("content_hash", ".bin")
        try {
            Files.write(file, data)
            assertEquals(ContentHash.of(data), ContentHash.of(file))
        } finally {
